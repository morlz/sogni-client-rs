...
mod tests {
    ...
    async fn account_switch_must_reauthenticate_the_socket() {
        ...
        let handshake = wire.recv().await.unwrap();
    ...
    async fn cookie_login_must_send_the_cookie_on_websocket_upgrade() {
        ...
        client
            ...
            .await
            .unwrap();
        let handshake = tokio::time::timeout(Duration::from_secs(3), wire.recv())
            .await
            .unwrap()
            .unwrap();
        client.abort();
    ...
    async fn stale_refresh(fail: bool, close: bool) {
        ...
        tokio::time::timeout(Duration::from_secs(2), begun.notified())
            .await
            .unwrap();
        let second = token("account-b", now() + 3600.0);
        if close {
    ...
    async fn idle_sse_must_time_out() {
        ...
        let result = tokio::time::timeout(Duration::from_secs(2), events.next())
            .await
            .unwrap();
