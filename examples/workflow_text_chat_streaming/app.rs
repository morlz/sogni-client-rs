...
#[derive(Parser)]
#[command(about = "Streaming Sogni chat with thinking-block filtering")]
struct Args {
...
pub async fn run() -> Result<()> {
    ...
    if !args.execute || args.dry_run {
        ...
        println!(
            ...
            serde_json::to_string_pretty(&shared::runtime::request(&settings, &messages, true))?
        );
        return Ok(());
    }
    if prompt.is_empty() {
    ...
    let client = common::auth::connect(
        common::auth::unique_app_id("sogni-chat-streaming"),
        Network::Fast,
    )
    .await?;
    ...
    let outcome = shared::stream_response(&client, &request, args.show_thinking).await;
    reporter.abort();
    if let Ok((_, result, ttft)) = &outcome {
        shared::completion_summary(result, started.elapsed(), Some(*ttft));
        if result.finish_reason == "length" && settings.think {
            eprintln!("Thinking exhausted max_tokens; increase --max-tokens or use --no-think.");
        }
    }
